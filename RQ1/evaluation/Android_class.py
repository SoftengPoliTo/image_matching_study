...
from Csv_writer import read_all, read_dim
...
def _build_word_sequence(text, content_desc, resource_id):
  ...
  """
    if text or content_desc:
        # re.findall(r"[\w']+|[?.!/,;:]", text if text else content_desc)
...
def _build_etree_from_json(root, json_dict):
  ...
  """
    ...
    root.set('text', json_dict.get('text', '').__str__())
    root.set('resource-id', json_dict.get('resource-id', ''))
    content_desc = json_dict.get('content-desc', [None])
    # content_desc[0].replace('\x00', '')
    root.set(
        'content-desc', '' if content_desc[0] is None else content_desc[0].__str__())
...
class ViewHierarchy(object):
    ...
    """
        screen_width, screen_height = read_dim("device_dimension.csv", device_name)
    ...
    """
        ...
        self._dom_location_dict, self._element_path = self._calculate_dom_location()
    ...
    def get_leaf_nodes(self):
        ...
        list = [
            LeafNode(element, self._dom_location_dict[id(element)], self._element_path[id(element)],
                     self._screen_width, self._screen_height)
            for element in self._all_visible_leaves
        ]
        only_final_nodes = []
        only_final_nodes_bounds = []
        # retrieved elements on the top of screen

        for i in list:
            found = False
            bounds_i = i.uiobject.bounding_box

            for j in only_final_nodes_bounds:
                if j.x1 < bounds_i.x1 and j.x2 > bounds_i.x2 and j.y1 < bounds_i.y1 and j.y2 > bounds_i.y2:
                    index_to_overwrite = only_final_nodes_bounds.index(j)
                    only_final_nodes_bounds[index_to_overwrite] = bounds_i
                    only_final_nodes[index_to_overwrite] = i
                    found = True

            if not found:
                only_final_nodes.append(i)
                only_final_nodes_bounds.append(bounds_i)

        return only_final_nodes

    def get_target_node(self, resource_value):
        """Returns a list of all the leaf Nodes."""
        list = [
        ...
        for i in list:
            ...
            bounds_i = i.uiobject.bounding_box
            for j in only_final_nodes_bounds:
                if (resource_value in i.uiobject.resource_id or resource_value in i.uiobject.content_desc
                        or resource_value in i.uiobject.text):
                    index_to_overwrite = only_final_nodes_bounds.index(j)
                    only_final_nodes_bounds[index_to_overwrite] = bounds_i
                    only_final_nodes[index_to_overwrite] = i
                    found = True

            if not found and (resource_value in i.uiobject.resource_id
                              or resource_value in i.uiobject.content_desc
                              or resource_value in i.uiobject.text):
                only_final_nodes.append(i)
                only_final_nodes_bounds.append(bounds_i)
        if only_final_nodes != []:
    ...
    """
        ...
        all_visible_leaves = [
            element for element in all_elements if self._is_leaf(element)
                                                   and self.is_clicable_visible_enabled(element)
    ...
    """
        ...
        elements_path = collections.defaultdict(lambda: [None])
        ...
        for element in self._all_visible_leaves:
            ...
            elements_path[id(element)] = str_tmp
        ...
        return dom_location_dict, elements_path
    ...
    def is_clicable_visible_enabled(self, element):
        text = element.get('text')
        content_desc = element.get('content-desc')
        clickable = element.get('clickable')
        # and
        if clickable == "true" or ((text != None and text != "") or (content_desc != None and content_desc != "")):
            return True
        else:
            return False
